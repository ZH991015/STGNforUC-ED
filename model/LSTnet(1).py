...
class LSTNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(0, seq_len - skip_step + 1, skip_step):
            ...
            # Ensure skip_out_i matches the original sequence length
            skip_out.append(skip_out_i)

        # Combine all skip_out parts to form the full sequence
        if len(skip_out) > 0:
            skip_out = torch.cat(skip_out, dim=1)
            skip_out = self.skip_linear(skip_out)
        else:
            skip_out = torch.zeros((batch_size, seq_len, self.output_dim)).to(x.device)

        # Ensure the output dimensions are (batch_size, seq_len, output_dim)
        out = out[:, -seq_len:, :]  # Take only the last `seq_len` outputs if necessary
        combined_out = out + skip_out

        # Highway Network for AR
        highway_out = self.highway(combined_out)
        # Autoregressive component
        ar_out = x  # Use the entire input sequence
        ar_out = ar_out.permute(0, 2, 1)  # (batch_size, num_features, seq_len)
        ar_out = self.ar(ar_out)  # Apply the autoregressive component
