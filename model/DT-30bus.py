...
newy = pd.read_csv("./all_a_array_2.csv",header=None)
newy= newy.values
...
# 展平数据，准备训练
X = expanded_x.reshape(-1, 288*30)  # 展平成二维数组，每个样本30个特征
y = newy.reshape(-1, 288*6)  # 展平成二维数组，每个样本6个输出
...
# 处理多输出回归问题
multioutput_dt = MultiOutputRegressor(dt_model)
...
y_pred = multioutput_dt.predict(X_test)
end_time = time.time()  # 记录预测结束时间
prediction_time = end_time - start_time  # 计算预测耗时
print(f"Prediction time: {prediction_time} seconds")

# 计算均方误差和绝对误差用于评估
mse = mean_squared_error(y_test, y_pred)#11.4
mae = mean_absolute_error(y_test, y_pred)#1.62
