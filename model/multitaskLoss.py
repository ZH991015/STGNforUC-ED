...
# 加载.pth文件
# checkpoint = torch.load('./best_model_epoch_280_loss_0.1192.pth')  #0.09
# checkpoint = torch.load('./best_model_MAE/best_model_epoch_296_loss_0.0795.pth') #0.089
# checkpoint = torch.load('./best_model_epoch_368_loss_0.0791.pth') #0.095
# checkpoint = torch.load('./best_model_epoch_388_loss_0.0777.pth') #0.095
# checkpoint = torch.load('./best_model_epoch_345_loss_0.0806.pth') #0.096
checkpoint = torch.load('./best_model_MAE/best_model_epoch_395_loss_0.0786.pth')#0.082
...
# 定义惩罚项的系数
sigma1 = nn.Parameter(torch.tensor(1.0))
sigma2 = nn.Parameter(torch.tensor(1.0))
# 定义修改后的损失函数
...
optimizer.add_param_group({
    ...
    'lr': 0.1  # 为 sigma 参数设置不同的学习率
})

scheduler = CosineAnnealingLR(optimizer, T_max=100, eta_min=0)
# 迭代数据加载器
best_val_loss =0.078# 初始化最佳验证损失为无穷大
...
for epoch in range(100):  # number of epochs
    ...
    if val_loss < best_val_loss:  # 检查这个epoch的验证损失是否是目前为止最低的
        ...
        if counter >= patience:
            ...
            break
    print(f"Epoch: {epoch}, Training Loss: {train_loss:.4f}, Validation Loss: {val_loss:.4f}, R2 Score: {r2:.4f},Current Learning Rate: {current_lr:.6f},sigma1:{sigma1:.4f},sigma2: {sigma2:.4f}")
